...
var SkuManage = /** @class */ (function() {
  ...
  SkuManage.prototype.excuteBySeleted = function(activeSpuTagMap) {
    var _this = this;
    // 从小到大排序
    var activeSpuTagMapKeyList = Object.keys(activeSpuTagMap)
    ...
    for (var i = 0; i < activeSpuTagMapKeyList.length; i++) {
      ...
      nextEmptyKV = nextEmptyKV.concat(
        currentList.reduce(function(t, item) {
          var currentSpuTagMapKey = item
  ...
  SkuManage.prototype.computeAllCaseInfo = function() {
    ...
    var countArrTemp = [];
    var spuDIdTemp = [];
    var mArr = this.skuParamVoList.map(function(item) {
      return item.valueList.length;
    });
    var skuParamVoListLen = this.skuParamVoList.length;
    var _loop_1 = function(index) {
      ...
      caseCom.forEach(function(v) {
        ...
        countArrTemp = [];
        spuDIdTemp = [];
        includeIndexArrTemp = intersectionSortArr.apply(
          void 0,
          v.map(function(vv) {
            return _this.keyRankMap[vv] || [];
          })
        );
        ...
        _this.indexKeyInfoMap[index][v.join(joinAttrStr)] = {
          ...
          // 转为数字
          priceArr: priceArrTemp.map(function(item) {
            return +item;
          }),
          totalCount: countArrTemp.reduce(function(t, c) {
            return t + c;
          }, 0)
        };
      });
  ...
  SkuManage.prototype.computeEmptyInfo = function(arrKeyCount, activeSpuTagMapKey) {
    ...
    var nextEmptyKV = [];
    if (arrKeyCount === 0) {
      return Object.keys(this.indexKeyInfoMap[0]).filter(function(item) {
        return _this.indexKeyInfoMap[0][item].totalCount === 0;
      });
    }
    ...
    if (nextEmptyKeyArr.length) {
      ...
      nextEmptyKV = uniqueArr(
        nextEmptyKeyArr.map(function(item) {
  ...
  SkuManage.prototype.getSelectedIndexKeyArr = function(activeSpuTagMap) {
    // paramId从小到大排序
    return Object.keys(activeSpuTagMap)
      .filter(function(key) {
